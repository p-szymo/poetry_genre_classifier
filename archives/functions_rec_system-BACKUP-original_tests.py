...
def poem_similarity(title, df, model, num_poems=5, to_print=True):
    ...
    similar_poems = model.docvecs.most_similar([vec], topn=num_poems+1)[1:]
...
def poem_similarity(
    ...
    similar_poems : list (tup)
        List of similar poems with poem index as an interger
    ...
    # find and return poems that are most similar to the input poem
    # add one to the input number of poems and slice off first result because the first result will always be
    # the same as the input poem
    similar_poems = sorted(cos_sims, key=itemgetter(1), reverse=True)[1:num_poems+1]
